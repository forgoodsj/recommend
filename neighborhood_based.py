...
def Recommend(user,train,W):
    ...
    :param user: 用户编号
    :param train: 格式{user1:{movie1:rank1,movie2:rank2},user2:{movie33:rank1,movie4:rank2}}
    :param W: 格式{user1:{user2:score2,user3:score3},user2:{user1:score1,user3:score3},user3:{user1:score1,user2:score2}}
    :return: {movie3:rank3,movie5:rank5}
    '''
    rank =dict()
    interacted_items = train[user] #用户已经评分过的电影

    for v, wuv in sorted(W[user].items(),key=operator.itemgetter(1),reverse=True)[0:3]: #取与当前用户相似度最高的前3个用户，这里的3可以调整
